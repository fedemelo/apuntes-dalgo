...
def rod_cut_top_down(n: int, prices: arreglo[int]) -> int:
    |\br{arri}|optimal_revs = |\text{arreglo de }|int|\text{ de longitud $n+1$}|
    optimal_revs[0] = 0
    for i=1 to n:
...
def rod_cut(n: int, prices: arreglo[int]) -> pila[int]:
    optimal_revs = |\text{arreglo de }|int|\text{ de longitud $n+1$}|
    optimal_revs[0] = 0
    optimal_cuts = |\text{arreglo de }|int|\text{ de longitud $n+1$}|
    ...
    segments = |\text{pila de }|int|\text{ de longitud máxima $n$}|
    |\br{cortsi}|i = n
    while i > 0:
        cut = optimal_cuts[i]
        segments.put(cut)
