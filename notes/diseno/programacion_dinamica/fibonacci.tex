...
Su ineficiencia radica en que presenta subproblemas superpuestos: para calcular $F(n)$ se necesitan calcular $F(n-1)$ y $F(n-2)$; para calcular $F(n-1)$ se necesitan calcular $F(n-2)$ y $F(n-3)$. En tan solo dos pasos, ya se ha calculado $F(n-2)$ dos veces. A medida que se avanza, la cantidad de cálculos repetidos crece exponencialmente. De hecho, la complejidad temporal de ese algoritmo es $\Theta(\phi^n)$, donde $\phi$ es el número áureo, aproximadamente 1.618.
...
Una primera optimización es la implementación top-down: incluir memoización en la implementación recursiva para eliminar la redundancia que introducen los problemas superpuestos:
\begin{pseudocode}
función fibonacci_top_down(n: entero) entero
    ...
    return fibonacci_top_down_recursion(n, |\bxl{auxf}|soluciones)|\bxr{auxf}|

función fibonacci_top_down_recursion(n: entero, soluciones: arreglo de enteros) entero
