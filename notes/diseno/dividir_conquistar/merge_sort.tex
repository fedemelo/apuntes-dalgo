...
Los primeros dos pasos son muy sencillos porque en algún punto al dividir por la mitad se obtienen arreglos de longitud uno, que ya no hay que ordenar. Entonces, si encapsulamos el paso de combinar, este es el seudocódigo:
\begin{pseudocode}
...
\end{pseudocode}
\begin{annotations}
    \codebrace[xshift=2.6cm, width=8cm]{gi}{gf}{No se retorna nada porque el algoritmo es in-place, pero se necesita el retorno como caso base de la recursión}
\end{annotations}
...
\begin{pseudocode}
merge_sort(números, 0, números.longitud-1)
\end{pseudocode}

Nótese que el arreglo no se divide físicamente en cada llamado recursivo; únicamente se delimitan distintas regiones mediante índices. En cada llamado recursivo ocurre lo siguiente:
\begin{enumerate}
    ...
    \item Una vez ambas llamadas retornan, se ejecuta \inline{combinar}, que mezcla ambas mitades ordenadas \emph{in-place} dentro del rango delimitado por \inline{ind_min} e \inline{ind_max}. 
\end{enumerate}
