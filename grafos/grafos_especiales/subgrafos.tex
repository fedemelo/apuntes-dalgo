...
\begin{center}
  \begin{tikzpicture}[vertex/.style={circle, fill=black, inner sep=2pt}]
    ...
    \end{scope}
  \end{tikzpicture}
  \captionof{figure}{Un grafo, un subgrafo inducido y un subgrafo (no inducido).}
  \label{fig:grafo_subgrafo_inducido}
\end{center}
...
\subsection{Sumersión}
\label{sec:sumersion}

El concepto de sumersión generaliza la noción de desigualdad débil en cardinalidad de conjuntos, \(|A|\leq |B|\). 

\begin{definicion}{}{}
  Se dice que \(G\) está \emph{sumergido} es \(H\) si es isomorfo con algún subgrafo de \(H\).
\end{definicion}
Es decir, \(G\) está sumergido en \(H\) si existe una función \emph{inyectiva} \(f\colon V(G) \to V(H)\) tal que para todos \(u,v \in V(G)\), si \(u\longrightarrow v\) entonces \(f(u) \longrightarrow f(v)\).
...
Esta definición no es universal. Si en otro documento se de esa definición, pero reza \say{\(u\longrightarrow v\) si y solamente si \(f(u) \longrightarrow f(v)\)}, entonces se alude a una noción similar, pero \(G\) es isomorfo a un subgrafo \emph{inducido}.