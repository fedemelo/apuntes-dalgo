...
A continuación se define el vocabulario que comúnmente se utiliza al trabajar con grafos. Para esta sección, sea $G=(V,E)$ un grafo simple y sean $u, v \in V$ vértices.

\subsubsection{Orden y tamaño}

El \concept{orden} de $G$, denotado por $|G|$, se define como el número de vértices: $$|G| = |V|.$$ Es usual usar $n$ para $n = |G|$.
...
Para grafos \emph{no dirigidos}, se dice que un arco $\{u,v\}$ (o simplemente $uv$) \concept{une} (\textit{joins}) los vértices $u$ y $v$. Se dice que $u$ y $v$ son \concept{incidentes} a el arco $uv$, al igual que el arco $uv$ es \concept{incidente} tanto a $u$ como a $v$.
...
\begin{center}
  \begin{tikzpicture}[scale=1,
    ...
    % Bottom face (outer square): x (down-left), u (down-right), v (up-right), w (up-left)
    \node[vertex, label=below left:$x$]  (x) at (0,   0)   {};
    \node[vertex, label=below right:$u$] (u) at (3,   0)   {};
    \node[vertex, label=right:$v$]       (v) at (3,   3)   {};
    \node[vertex, label=above left:$w$]  (w) at (0,   3)   {};
    % Top face (inner square, offset up-right): d (down-left), a (down-right), b (up-right), c (up-left)
    \node[vertex, label=below:$d$]       (d) at (1.5, 1.5) {};
    \node[vertex, label=right:$a$]       (a) at (4.5, 1.5) {};
    \node[vertex, label=above right:$b$] (b) at (4.5, 4.5) {};
    \node[vertex, label=above:$c$]       (c) at (1.5, 4.5) {};
    % Bottom face edges
    \draw (x) -- (u) -- (v) -- (w) -- (x);
    % Top face edges
    \draw (d) -- (a) -- (b) -- (c) -- (d);
