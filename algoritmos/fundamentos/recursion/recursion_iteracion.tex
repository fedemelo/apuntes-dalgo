...
Recuérdese que todos los tipos de ciclos se pueden generalizar mediante la instrucción \inline{while}, como se muestra en el apéndice \ref{apx:iteracion}.

A medida que el ciclo \inline{while} ejecuta, puede modificar el \concept{estado del programa}, denotado por \(\sigma\): el conjunto de valores de las variables cuyo contenido puede afectar la ejecución del algoritmo. El estado determina si la condición booleana del \inline{while} es verdadera, por lo que dicha condición se denota en función del estado como \(B(\sigma)\). Con eso en mente, la forma general de cualquier \inline{while} (y por ende de cualquier ciclo) se puede escribir como:
\begin{codeblock}[Abstracción semántica de cualquier ciclo.]
\begin{pseudocode}
while |\(B(\sigma)\)|:
    |\(\sigma\)| = |\(T(\sigma)\)|
\end{pseudocode}
...
\begin{codeblock}[Equivalente recursivo de cualquier ciclo.]
\begin{pseudocode}
loop(|\(\sigma\)|):
    if not |\(B(\sigma)\)|:
        return |\(\sigma\)|
