...
Los índices \inline{lo} y \inline{hi} como parámetros son necesarios porque indican qué segmento del arreglo se está procesando. La invocación inicial, que resuelve el problema completo, usa los índices extremos del arreglo: \inline{merge_sort(nums, 0, len(nums)-1)}. Nótese que el arreglo realmente no se divide en cada llamado recursivo, sino que se delimitan segmentos mediante índices. Eso permite que el algoritmo sea \concept{\textit{in-place}}: modifique directamente el arreglo original en lugar de retornar uno nuevo.
