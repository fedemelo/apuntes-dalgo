...
def max_subarray_brute_force(nums: arreglo[int]) -> tuple[int, int, int]:
    ...
    for lo=0 to len(nums)-1:
        sub_sum = 0
        |\br{sumi}|for hi=lo to len(nums)-1:
            sub_sum += nums[hi]|\br{sumf}|
...
\end{pseudocode}
\begin{annotations}
    \codebrace[xshift=1.5cm,width=8cm]{cmpi}{cmpf}{Cada subarreglo de \(lo\) hasta \(hi\) se compara contra el mejor visto hasta el momento; no se espera a que termine el ciclo interno para comparar}
\end{annotations}
