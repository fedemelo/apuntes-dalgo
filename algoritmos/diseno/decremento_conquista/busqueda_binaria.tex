...
\begin{codeblock}[Búsqueda binaria recursiva.][impl:busqueda_binaria_recursiva]
\begin{pseudocode}
def binary_search_rec(nums: arreglo[int], target: int, lo: int, hi: int) -> int:
    |\br{mi}|if hi < lo:
        return -1|\br{mf}|

    |\bxl{over}|mid = (hi - lo) // 2 + lo|\bxr{over}|
...
\begin{annotations}
  ...
  \codenote[xshift=0.5cm, width=12cm]{over}{\annMitadSinOverflow{lo}{hi}}
  \codebrace[xshift=0.3cm, width=7cm]{gtb}{gtf}{El objetivo es mayor: debe estar a la derecha. El llamado recursivo va desde después de la mitad hasta el final del segmento.}
  \codenote[xshift=0.5cm, width=12cm]{eq}{Si no es ni mayor ni menor, es igual: el objetivo está en el índice del medio.}
\end{annotations}
\end{codeblock}
...
\begin{codeblock}[Invocación inicial de búsqueda binaria recursiva.][impl:busqueda_binaria_invocacion]
\begin{pseudocode}
...
\begin{codeblock}[Búsqueda binaria iterativa.][impl:busqueda_binaria_iterativa]
\begin{pseudocode}
...
\end{pseudocode}
\begin{annotations}
  \codebrace[xshift=1.5cm, width=8.5cm]{ib}{if}{El objetivo es menor que el valor del medio: debe estar a la izquierda. Se mueve el extremo derecho a justo antes de la mitad.}
\end{annotations}
\end{codeblock}
