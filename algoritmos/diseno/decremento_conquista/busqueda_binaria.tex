...
Ese peor caso ocurre cuando el objetivo no se encuentra, pero resulta que también ocurre en el caso promedio: \(T_\text{promedio} \in \Theta(\log n)\). Se omite la demostración matemática, pero la intuición es que los pasos de la búsqueda binaria se pueden diagramar como un árbol binario, y en todo árbol binario la mayoría de los nodos están cerca de las hojas. Por ende, en la mayoría de los casos, el algoritmo tendrá que ejecutar la máxima o cerca a la máxima cantidad de pasos.
...
El análisis asintótico de complejidad es el mismo de antes, pues se puede aplicar el mismo racicionio para el máximo número de pasos. No obstante, esta implementación es ligeramente más eficiente al evitar el \hyperref[sec:sobrecosto_recursion](sobrecosto de la recursión).
